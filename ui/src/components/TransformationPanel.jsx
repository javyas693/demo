...
export default function TransformationPanel({ summary, trace }) {
  const capital_released = summary.capital_released || 0;
  const allocation_to_income = summary.allocation_to_income || 0;
  const allocation_to_model = summary.allocation_to_model || 0;
    ...
    <div className="flex-1 flex flex-col items-center justify-center relative z-0 px-2 py-8">
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-xl shadow-sm w-full p-4 flex flex-col items-center z-10 relative">
        <div className="absolute -top-3 bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">
          Capital Reallocated from Concentrated Position
        </div>
        
        <p className="text-3xl font-black text-emerald-600 mt-2 mb-1">${capital_released.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
        <p className="text-xs text-slate-400 mb-4">Total released to date</p>
